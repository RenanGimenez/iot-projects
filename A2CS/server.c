...
int main(int argc, char **argv)
{
	...
	/* Sending file data */
	while (((sent_bytes = sendfile(peer_socket, fd, &offset, BUFSIZ, len)) > 0) && (remain_data > 0))
	{
...
int sendfile(int peer_socket, int fd, int *offset, int bufsiz, int file_size){
	if(*offset >= file_size)
		...
		ssize_t result = read(fd, &buffer,endPoint-*offset-2);
		printf("\tresult");
		fwrite(buffer, result, 1, stdout);
		printf("\n");
		offset += result;
		send(peer_socket, buffer, result,0);
		return result;
	}
